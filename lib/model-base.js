...
var ModelBase = Class.extend({
  ...
  get:function(){// jshint ignore:line
    ...
    var self = this;
    if(!this._checkColl()){
      return callback(this.errMsg);
    }
    if(!this._convertId(query)){
      return callback(this.errMsg);
    }
   ...
   * @return {null}
   */
  getList:function(){ // jshint ignore:line
      ...
      if(!this.coll.safe){
        ...
        for(var key in fields){
          ...
          if(typeof this[key] ==='string' && fields[key]==='file'){
            ...
            if(pObj.ext===''){
              return callback(key+' is not a file'+this.__className);
            }
            obj[key]=this[key];
          }else if(typeof this[key] !==fields[key]){
            return callback(key+
                            ' is not conformed with coll config:'+
                            this.__className);
          }else{
            obj[key]=this[key];
          }
   ...
   * @return {null}
   */
  delete:function(){ //jshint ignore:line
  ...
  /////////////////////////// private functions
  _getUpDir:function(){
    if(!this.hasOwnProperty('__upDir')) {
  ...
  _init:function(){
    ...
    this._makeUploadDir();
    this._setCollection();
    this.__proto__._bInit=true; // jshint ignore:line
  },
  ...
  _setCollection:function(){
    ...
    if(!this._checkColl()) {
      this._clearErrMsg();
      logger.info('not creating database'+
                  'table because of lack of coll definition');
      return;
    }
    // jshint ignore:start
  ...
  _makeUploadDir: function(){
    ...
    if(fs.existsSync(upDir)){
      ...
      if(s.isDirectory()) {
        ...
        throw 'creating upload directory failed:'+this.__classname+':'+upDir;
      }
    }
  ...
  _rmOneOldFile:function(filedesc){
    ...
    if(typeof filedesc.path !== 'string'){
      return;
    }
  ...
  _processOne:function(field,filedesc){
    ...
    // saved into database
    var collfields = this.__proto__.coll.fields; // jshint ignore: line
    if(!isEmpty(collfields) && !collfields[field]){
      collfields[field]='file';
    }
  },
  ...
  _objConverter:function(obj){
    if(obj){
      obj.__proto__ = this.__proto__; // jshint ignore:line
      obj.init();
    }
    return obj;
  },
    ...
    if(obj._id){
      ...
      this.collection.insertOne(obj,function(err,writeOpRes){
        ...
        self.collection.findOne(
          {_id:writeOpRes.ops[0]._id},
          self._cbConverter(callback));
      });
    }
  },
});
